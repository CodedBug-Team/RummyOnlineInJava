    ...
    window: function(options){
        ...
        if(options.buttons && options.buttons.length > 0){
            ...
            $A(options.buttons).each(function(button){
            	...
            	if (!button.id){
                    var but = new Element('button', {className:'big-button buttons buttons-'+color, type:'button', name:button.name}).observe('click', function(){
                        button.handler(win, but);
                    });
            	}else{
                    var but = new Element('button', {className:'big-button buttons buttons-'+color, type:'button', name:button.name, id:button.id}).observe('click', function(){
                        button.handler(win, but);
                    });
            	}
...
Protoplus.ui = {
    ...
    softScroll: function(element, options){
        ...
        container.hover(function(){
            setScrollSize(); // Re calculate scroll size on each focus
    ...
    bigSelect: function(element, options){
        ...
        if(Prototype.Browser.IE /*&& Protoplus.getIEVersion() < 8 || Prototype.Browser.IE9*/){
